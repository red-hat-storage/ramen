...
func (v *VRGInstance) validateVRStatus(volRep *volrep.VolumeReplication, state ramendrv1alpha1.ReplicationState) bool {
	...
	if state == ramendrv1alpha1.Primary {
		validated, condState := v.validateVRValidatedStatus(volRep)
...
func (v *VRGInstance) validateVRValidatedStatus(
	...
	if !conditionMet && condState != conditionMissing {
		...
		v.updatePVCDataReadyConditionHelper(volRep.Namespace, volRep.Name, VRGConditionReasonError, errorMsg,
			defaultMsg)
		v.updatePVCDataProtectedConditionHelper(volRep.Namespace, volRep.Name, VRGConditionReasonError, errorMsg,
			defaultMsg)
